...
Cross-model reporting requires at least three usable AI providers. A provider is usable when at least half of its expected captures succeed. Failed or unavailable providers remain visible as excluded evidence, are never counted as "not retrieved," and do not block the report once the three-provider quorum is met. If fewer than three providers are configured or usable, Riseklix pauses instead of generating an automated cross-model report.
