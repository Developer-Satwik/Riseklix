...
export async function createProject(formData: FormData) {
  ...
  let { data: workspace } = await supabase.from('workspaces').select('id').limit(1).maybeSingle()

  if (!workspace) {
    const slug = `workspace-${crypto.randomUUID().slice(0, 8)}`
    const created = await supabase.from('workspaces').insert({ name: 'My Workspace', slug, created_by: userId }).select('id').single()
    if (created.error) redirect(`/projects/new?error=${encodeURIComponent(created.error.message)}`)
    workspace = created.data
    const member = await supabase.from('workspace_members').insert({ workspace_id: workspace.id, user_id: userId, role: 'owner' })
