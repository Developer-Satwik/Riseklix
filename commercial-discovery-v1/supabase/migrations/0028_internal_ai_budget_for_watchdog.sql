-- Split AI budget consumption into an authenticated user wrapper and a
-- service-role-only internal wrapper so the durable Autopilot watchdog can
-- resume paid stages without impersonating a user.

create or replace function private.consume_ai_budget_core(
  p_workspace_id uuid,
  p_kind text,
...
begin
  ...
  values (p_workspace_id, v_bucket)
  ...
  where workspace_id = p_workspace_id and bucket_start = v_bucket
  for update;
...
revoke all on function private.consume_ai_budget_core(uuid,text,integer) from public;
...
begin
  ...
  return private.consume_ai_budget_core(v_workspace, p_kind, p_units);
end;
$$;

create or replace function public.consume_ai_budget_internal(
  p_project_id uuid,
  p_kind text,
  p_units integer default 1
)
...
revoke all on function public.consume_ai_budget_internal(uuid,text,integer) from public;
revoke all on function public.consume_ai_budget_internal(uuid,text,integer) from anon;
revoke all on function public.consume_ai_budget_internal(uuid,text,integer) from authenticated;
grant execute on function public.consume_ai_budget_internal(uuid,text,integer) to service_role;
